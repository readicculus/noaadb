...
class IRBackgroundReviewCreateTask(luigi.Task):
    ...
    def run(self):
        ...
        images = s.query(IRImage)\
            ...
            .filter(~ exists().where(IRVerifiedBackground.ir_event_key==IRImage.event_key))\
            .filter(~ exists().where(IRUncertainBackground.ir_event_key==IRImage.event_key))\
            .filter(IRImage.event_key.ilike("%kotz%")).limit(1000).all()
...
if __name__ == '__main__':
    ...
    luigi.build([IRBackgroundReviewCreateTask()], local_scheduler=True)
